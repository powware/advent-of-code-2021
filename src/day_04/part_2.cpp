...
struct BingoBoard
{
    ...
    std::optional<int> Mark(int number) noexcept
    {
    ...
    bool ValidateRow(size_t row) const noexcept
    {
        size_t start = row * kWidth;
        return std::accumulate(&entries_[start], &entries_[start] + kWidth, 0) == -5;
    }

    bool ValidateColumn(size_t column) const noexcept
    {
        int sum = 0;
...
int main()
{
    const auto path = std::filesystem::current_path() / "input/day_04.txt";
