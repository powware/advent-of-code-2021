...
int main()
{
    const auto path = std::filesystem::current_path() / "input/day_07.txt";
    ...
    for (unsigned int i = min_distance; i <= max_distance; ++i)
    {
        ...
        {
            const unsigned int old_fuel = unsigned(std::abs(int(distance) - int(i)));
            fuel += (old_fuel * (old_fuel + 1) / 2) * occurances;
        }
