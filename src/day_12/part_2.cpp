...
void FindPaths(std::vector<Path> &paths, Path path, const Cave &cave, const Cave &end)
{
    ...
    for (auto &neighbor : cave.neighbors)
    {
        ...
        else if (neighbor.get().name != "start" && neighbor.get().name != "end" && !path.small_cave_visited_twice)
        {
            path.small_cave_visited_twice = true;
            FindPaths(paths, path, neighbor, end);
            path.small_cave_visited_twice = false;
        }
    }
}
